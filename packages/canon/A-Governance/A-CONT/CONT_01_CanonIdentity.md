...
> **Authority:** Canon Identity & Cell Registration Standard
...
- **v2.2.0** - Canon Planes & Prefixes Expansion (Major Update ✅)
  ...
  - ✅ **Standardized all 5 plane tables** with columns: Plane, ID Prefix, Type, Pattern, Definition, Example, Conflict Resolution, Source of Reference
  ...
  - ✅ Added "Forbidden" sections to Planes C and D
  - ✅ Updated repository layout (Section 4) to reflect new 5-plane structure with `canon/contracts/` and `canon/registry/`
  ...
  - ✅ Added explicit **CONT_01** identity block at top
  - ✅ Fixed types path to `packages/canon/src/types.ts` (import as `@aibos/canon`)
  - ✅ **Master Summary Table** (Section 3.8) with all 14 prefixes and conflict resolutions
  - 🟢 **CONT_01 – Execution Version** - Enterprise-grade, Fortune-500 caliber standard
...
| Plane | ID Prefix | Type     | Pattern           | Definition | Example | Conflict Resolution | Source of Reference |
| ----- | --------- | -------- | ----------------- | ---------- | ------- | ------------------- | ------------------- |
| A     | CONT      | Contract | `CONT_[0-9]{2,3}` | **Canon Law.** Immutable architectural standards and policy documents. Non-negotiable. | `CONT_01_CanonIdentity.md` | **CONT vs REF:** CONT is internal law; REF is external evidence. CONT may cite REF. | `canon/contracts/` |
| A     | ADR       | Decision | `ADR_[0-9]{3}`    | **Architecture Decision Record.** Immutable snapshots of technical choices with context, decision, and consequences. | `ADR_005_SwitchToTurborepo.md` | **ADR vs REF:** ADR is the decision; REF is supporting research. ADR links to REF in "Context" section. | `canon/contracts/adrs/` |

**Versioning:** Contracts use SemVer. ADRs are immutable snapshots (no versioning after creation).
...
| Plane | ID Prefix | Type      | Pattern | Definition | Example | Conflict Resolution | Source of Reference |
| ----- | --------- | --------- | ------- | ---------- | ------- | ------------------- | ------------------- |
| B     | PAGE      | Page      | `PAGE_<PageCode>_<ShortName>` or `<PageCode>` (e.g. `META_02`) | **Routable Screen.** A route/entry point that users navigate to. Core code is `META_02`, `PAY_01`, etc. | `PAGE_META_02_GodView` or `META_02` | **PAGE vs SPEC:** PAGE is the implementation (code); SPEC is the requirement (document). SPEC is written first, PAGE implements it. | `apps/web/canon-pages/` |
| B     | COMP      | Component | `COMP_<ComponentCode>_<ShortName>` or `<ComponentCode>` (e.g. `TBLM01`) | **Reusable UI Component.** Governed UI elements (tables, forms, widgets). Must be used instead of raw library imports. | `COMP_TBLM01_MonetizeFullTable` or `TBLM01` | **COMP vs Utility:** COMP is a governed component with meta block. Utilities are ungoverned helpers. | `packages/ui/canon/` |
| B     | CELL      | Cell      | `CELL_<PAGE_CODE>_<SCHEMA_NUM>_<TAB_NUM>_<POLICY_NUM_OR_NULL>` | **Atomic Governed Unit.** The smallest traceable unit: Page + Schema + Tab + Policy + Component. Used for debugging and audit. | `CELL_META_09_101_10_25` | **CELL uniqueness:** Each unique combination of Page/Schema/Tab/Policy creates a new Cell ID. Cell ID changes when any component changes. | `apps/web/canon-pages/.../cells/` |

**Code Mapping:**
* `PageCode` (e.g. `META_02`, `PAY_01`) → Filename: `META_02_CanonLandingPage.tsx`
* `ComponentCode` (e.g. `TBLM01`, `TBLL01`) → Filename: `TBLM01_MonetizeFullTable.tsx`
* `cell_id` compressed format: `CELL_META_09_101_10_25` → `META_09 / SCH_101 / TAB_10 / POLY_25`
...
| Plane | ID Prefix | Type     | Pattern | Definition | Example | Conflict Resolution | Source of Reference |
| ----- | --------- | -------- | ------- | ---------- | ------- | ------------------- | ------------------- |
| C     | ENT       | Entity   | `ENT_[0-9]{3}` | **Persistence Model.** The database table definition (Prisma/Drizzle/SQL). How data is *stored*. | `ENT_010_Payment` | **ENT vs SCH:** ENT is how we *store* it. SCH is how we *validate* input. SCH verifies data before it enters ENT. | `packages/database/schema.ts` |
| C     | SCH       | Schema   | `SCH_[0-9]{3}` | **Data Shape.** Zod/TS definitions for API payloads and forms. Runtime validation at the boundary. | `SCH_101_PaymentFlexFields` | **SCH vs TYPE:** Pure TS interfaces are just code (ungoverned). SCH implies *runtime validation*. | `packages/schemas/` |
| C     | POLY      | Policy   | `POLY_[0-9]{2,3}` | **Business Logic.** Executable rules (permissions, limits, workflows). Runs when users interact. | `POLY_25_PaymentMetadataPolicy` | **POLY vs CONFIG:** Policies execute logic. Configs are static values. **POLY vs TOOL:** POLY runs on user action; TOOL runs in CI/Dev. | `packages/policies/` |
| C     | CONST     | Constant | `CONST_[0-9]{2,3}` | **Static App Constants.** Fixed code constants that rarely change. Not database, not environment. | `CONST_01_SupportedCurrencies` | **CONST vs ENV:** CONST is code-level static data. ENV is infrastructure (un-ID'd). If it affects user behavior dynamically, it's POLY or ENT. | `packages/core/constants/` |

**Forbidden (Do NOT assign IDs to):**
* Pure TypeScript interfaces (`types.ts`) – use `SCH` only if runtime validation exists
* Environment variables (`.env`) – infrastructure, not architecture
* Framework configs (`package.json`, etc.) – standard industry files
...
| Plane | ID Prefix | Type      | Pattern | Definition | Example | Conflict Resolution | Source of Reference |
| ----- | --------- | --------- | ------- | ---------- | ------- | ------------------- | ------------------- |
| D     | TOOL      | Tool      | `TOOL_[0-9]{2,3}` | **Internal Scripts.** CLIs, scaffolders, maintenance scripts. Runs in CI/CD or on dev laptops, NOT on user actions. | `TOOL_01_CanonSync.ts` | **TOOL vs POLY:** TOOL runs in CI/CD or dev laptop. POLY runs when user clicks "Buy". If it affects runtime behavior, it's POLY. | `scripts/` |
| D     | MIG       | Migration | `MIG_<YYYYMMDD>_<ShortSlug>` | **DB State Changes.** Immutable SQL/Code that alters ENT structure. Never modified after creation. | `MIG_20241211_AddUserRole.sql` | **MIG vs ENT:** ENT is the *current* table definition. MIG is the *change* that created/modified ENT. MIG is immutable history. | `packages/database/migrations/` |
| D     | INFRA     | Infra     | `INFRA_[0-9]{2,3}` | **IaC Definitions.** Terraform, Docker, K8s manifests. Infrastructure as code for deployment. | `INFRA_01_RedisCluster.tf` | **INFRA vs CONFIG:** INFRA is IaC definitions (governed). Framework configs (`package.json`, `turbo.json`) are un-ID'd. | `infra/` |

**Forbidden (Do NOT assign IDs to):**
* Framework configs: `package.json`, `eslint.config.js`, `tsconfig.json`, `next.config.mjs`
* Container orchestration defaults (unless custom IaC)
* CI/CD pipeline configs (unless custom tooling)
...
| Plane | ID Prefix | Type          | Pattern | Definition | Example | Conflict Resolution | Source of Reference |
| ----- | --------- | ------------- | ------- | ---------- | ------- | ------------------- | ------------------- |
| E     | SPEC      | Specification | `SPEC_[0-9]{3}` | **Feature Dossiers.** Detailed functional requirements before they become code. What to build (requirements, scenarios, diagrams). | `SPEC_010_PaymentHub_v2.md` | **SPEC vs PAGE:** SPEC describes *what* to build (document). PAGE is *what was built* (code). SPEC is written first, PAGE implements SPEC. | `knowledge/specs/` |
| E     | REF       | Reference     | `REF_[0-9]{3}` | **External Library.** Third-party standards, research articles, PDFs. Static external material that supports decisions. | `REF_001_ISO27001.pdf` | **REF vs CONT:** REF is external evidence (library). CONT is internal law (binding). CONT may cite REF. **REF vs ADR:** REF is research; ADR is the decision that cites REF. | `knowledge/references/` |
...
* An **ADR** links to **REF** in its "Context" section
...
### 3.8 Master Summary Table

| Plane | ID Prefix | Type      | Pattern | Definition | Example | Key Conflict Resolution | Source of Reference |
| ----- | --------- | --------- | ------- | ---------- | ------- | ----------------------- | ------------------- |
| A     | CONT      | Contract  | `CONT_[0-9]{2,3}` | Canon Law. Immutable architectural standards. | `CONT_01_CanonIdentity.md` | CONT vs REF: CONT is law; REF is evidence. | `canon/contracts/` |
| A     | ADR       | Decision  | `ADR_[0-9]{3}` | Architecture Decision Record. Immutable snapshot. | `ADR_005_SwitchToTurborepo.md` | ADR vs REF: ADR is decision; REF is research. | `canon/contracts/adrs/` |
| B     | PAGE      | Page      | `<PageCode>` or `PAGE_<PageCode>_<Name>` | Routable Screen. User-facing entry point. | `META_02` / `PAGE_META_02_GodView` | PAGE vs SPEC: PAGE is code; SPEC is requirements. | `apps/web/canon-pages/` |
| B     | COMP      | Component | `<ComponentCode>` or `COMP_<Code>_<Name>` | Reusable UI Component. Governed, with meta block. | `TBLM01` / `COMP_TBLM01_MonetizeFullTable` | COMP vs Utility: COMP is governed; utilities are not. | `packages/ui/canon/` |
| B     | CELL      | Cell      | `CELL_<PAGE>_<SCH>_<TAB>_<POLY_OR_NULL>` | Atomic Governed Unit. Page+Schema+Tab+Policy. | `CELL_META_09_101_10_25` | Cell ID changes when any component changes. | `apps/web/canon-pages/.../cells/` |
| C     | ENT       | Entity    | `ENT_[0-9]{3}` | Persistence Model. Database table definition. | `ENT_010_Payment` | ENT vs SCH: ENT stores; SCH validates. | `packages/database/schema.ts` |
| C     | SCH       | Schema    | `SCH_[0-9]{3}` | Data Shape. Zod/TS runtime validation. | `SCH_101_PaymentFlexFields` | SCH vs TYPE: SCH has runtime validation; TYPE is abstract. | `packages/schemas/` |
| C     | POLY      | Policy    | `POLY_[0-9]{2,3}` | Business Logic. Executable rules. | `POLY_25_PaymentMetadataPolicy` | POLY vs CONFIG: POLY executes; CONFIG is static. | `packages/policies/` |
| C     | CONST     | Constant  | `CONST_[0-9]{2,3}` | Static App Constants. Fixed code values. | `CONST_01_SupportedCurrencies` | CONST vs ENV: CONST is code; ENV is infrastructure. | `packages/core/constants/` |
| D     | TOOL      | Tool      | `TOOL_[0-9]{2,3}` | Internal Scripts. Dev/Ops utilities. | `TOOL_01_CanonSync.ts` | TOOL vs POLY: TOOL runs in CI/dev; POLY runs on user action. | `scripts/` |
| D     | MIG       | Migration | `MIG_<YYYYMMDD>_<Slug>` | DB State Changes. Immutable SQL migrations. | `MIG_20241211_AddUserRole.sql` | MIG vs ENT: MIG is history; ENT is current state. | `packages/database/migrations/` |
| D     | INFRA     | Infra     | `INFRA_[0-9]{2,3}` | IaC Definitions. Terraform, Docker, K8s. | `INFRA_01_RedisCluster.tf` | INFRA vs CONFIG: INFRA is IaC; framework configs are un-ID'd. | `infra/` |
| E     | SPEC      | Spec      | `SPEC_[0-9]{3}` | Feature Dossiers. Requirements documents. | `SPEC_010_PaymentHub_v2.md` | SPEC vs PAGE: SPEC is requirements; PAGE implements. | `knowledge/specs/` |
| E     | REF       | Reference | `REF_[0-9]{3}` | External Library. Standards, research, PDFs. | `REF_001_ISO27001.pdf` | REF vs CONT: REF is external; CONT is internal law. | `knowledge/references/` |
