...
const CurrencyCell = ({ 
  ...
  if (value === null || value === undefined) {
    return <span className="text-[#444] font-mono">—</span>;
  }
...
const PercentageCell = ({ value }: { value: unknown }) => {
  ...
  return (
    <span className={cn(
      "font-mono text-sm",
      isNegative ? "text-[#FF4D4D]" : num > 0 ? "text-[#28E7A2]" : "text-[#888]"
    )}>
      {num >= 0 ? '+' : ''}{num.toFixed(2)}%
    </span>
  );
...
const StatusCell = ({ 
  ...
  const colorClass = mergedConfig[statusKey] || 'bg-[#1F1F1F] text-[#888] border-[#333]';
...
const BooleanCell = ({ value }: { value: unknown }) => {
  return (
    <div className="flex justify-center">
      {value ? (
        <BadgeCheck className="w-5 h-5 text-[#28E7A2]" />
      ) : (
        <div className="w-5 h-5 border border-[#333] rounded-sm bg-[#111]" />
      )}
    </div>
  );
};
...
const CodeCell = ({ value }: { value: unknown }) => {
  ...
  return (
    <span className="font-mono text-[11px] text-[#666] bg-[#111] px-1.5 py-0.5 rounded border border-[#222]">
      {String(value)}
    </span>
  );
};
...
const TextCell = ({ value }: { value: unknown }) => {
  ...
  return (
    <span className="text-[#CCC] text-sm truncate block" title={String(value)}>
      {String(value)}
    </span>
  );
};
    ...
    const colDef: ColumnDef<T, unknown> = {
        ...
        >
          ...
          {field.is_critical && (
            <span title="Critical Field">
              <Shield className="w-3 h-3 text-[#FFD600]" />
            </span>
          )}
        </div>
      ),
      ...
      cell: (info) => {
        ...
        switch (field.data_type) {
          case 'currency':
            return <CurrencyCell value={value} currency={field.format_pattern} />;
          ...
          case 'json':
            return (
              <code className="text-[10px] text-[#666] bg-[#111] px-1 rounded">
