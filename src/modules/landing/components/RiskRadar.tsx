...
export const RiskRadar = ({ activeRisks }: RiskRadarProps) => {
  ...
  return (
    <div
      className={cn(
        'relative mx-auto aspect-square max-h-[420px] w-full overflow-hidden rounded-xl border border-white/10 bg-[#000814] transition-all duration-700',
    ...
    >
      ...
      <div className="absolute left-4 top-4 z-20">
        <div
          className="flex items-center gap-2 rounded-full border bg-black/60 px-3 py-1.5 backdrop-blur-md"
        ...
        >
          {isCritical ? (
            ...
            <Shield className="h-3.5 w-3.5" style={{ color: theme.primary }} />
          ) : (
            <Scan className="h-3.5 w-3.5" style={{ color: theme.primary }} />
          )}
          <span
      ...
      <svg viewBox="0 0 540 540" className="h-full w-full">
        ...
        {[
          ...
          { r: 102, opacity: 0.2, width: 1.0 },
          { r: 153, opacity: 0.25, width: 1.0 },
          { r: 204, opacity: 0.3, width: 1.2 },
          { r: 255, opacity: 0.4, width: 1.5 },
        ].map((ring, index) => (
          <circle
            key={`ring-${index}`}
            cx="270"
          ...
          return (
            <motion.circle
              ...
              transition={{
                duration: 0.25,
                repeat: Infinity,
                repeatDelay: isCritical ? 0.4 : 4.75,
                ease: 'easeOut',
              }}
              filter="url(#intenseGlow)"
            />
