...
export const HybridRadar = ({
  ...
  return (
    <div
      className={cn(
        'group relative mx-auto aspect-square max-h-[420px] w-full overflow-hidden rounded-2xl border',
    ...
    >
      ...
      <div className="bg-gradient-radial absolute inset-0 from-transparent via-transparent to-black/80" />
      ...
      <motion.div
        ...
        style={{
          ...
          maskImage:
            'radial-gradient(circle, transparent 55%, black 65%, black 85%, transparent 100%)',
          WebkitMaskImage:
            'radial-gradient(circle, transparent 55%, black 65%, black 85%, transparent 100%)',
        }}
        animate={{ rotate: 360 }}
      ...
      <div className="absolute inset-0 z-10 flex items-center justify-center">
        ...
        >
          ...
          {/* Shield Icon Ring */}
          <div
            className="absolute -bottom-4 flex items-center gap-1.5 rounded-full px-3 py-1"
            style={{
          ...
          >
            {isCritical ? (
              ...
              <Shield className="h-3 w-3" style={{ color: theme.color }} />
            ) : (
              <Lock className="h-3 w-3" style={{ color: theme.color }} />
            )}
            <span
      ...
      <div className="absolute left-4 top-4 z-20">
        <div
          className="flex items-center gap-2 rounded-full border bg-black/70 px-3 py-1.5 backdrop-blur-md"
          style={{ borderColor: theme.color }}
        >
          <motion.div
      ...
      {/* 6. CORNER MARKERS */}
      {['top-4 right-4', 'bottom-4 right-4', 'bottom-4 left-4'].map(
        (pos, i) => (
          <div
            key={i}
            className={`absolute ${pos} h-6 w-6 border-l border-t opacity-20`}
            style={{ borderColor: theme.color }}
          />
        )
      )}
