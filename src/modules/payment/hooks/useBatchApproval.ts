...
export function validateBatchApproval(
  ...
  if (batch_rules.require_zero_anomalies && anomalyCount > 0) {
    blockReasons.push(
      `${anomalyCount} anomal${anomalyCount === 1 ? 'y' : 'ies'} require individual review`
    )
  }
  ...
  if (hasHighValue) {
    blockReasons.push(
      `Items over $${batch_rules.max_batch_amount.toLocaleString()} require individual approval`
    )
  }
...
export function useBatchApproval(
  ...
  const clusters = useMemo<FunctionalClusterData[]>(() => {
    ...
    return raw.map((c) => ({
      ...
      block_reason: c.can_batch_approve
        ? undefined
        : c.anomaly_count > 0
          ? `${c.anomaly_count} anomalies require review`
          : c.cluster_id === 'intercompany'
            ? 'IC transactions require settlement'
            : undefined,
    }))
  }, [])

  // Validate a specific cluster
  const validateCluster = useCallback(
    (clusterId: FunctionalCluster) => {
  ...
  const executeBatchApproval = useCallback(
    async (clusterId: FunctionalCluster) => {
...
export function useClusterSummary(payments: Payment[]) {
  return useMemo(() => {
    ...
    const summary = {
      ...
      byCluster: {} as Record<
        FunctionalCluster,
        { count: number; amount: number; anomalies: number }
      >,
      cleanClusters: 0,
      anomalousClusters: 0,
    }

    const clusters: FunctionalCluster[] = [
      'utilities',
      'logistics',
      'professional',
      'intercompany',
    ...
    clusters.forEach((cluster) => {
      const clusterPayments = pending.filter(
        (p) => p.functional_cluster === cluster
      )
      const anomalies = clusterPayments.filter(
