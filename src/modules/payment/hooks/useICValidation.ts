...
export function validateICTransaction(payment: Payment): ICValidationResult {
  ...
  const matchingPosition =
    IC_POSITIONS.find(
      (pos) =>
        pos.from_entity === payment.entity || pos.to_entity === payment.entity
    ) || null
...
export function useICValidation(
  payment: Payment | null
): ICValidationResult | null {
  return useMemo(() => {
    ...
    steps: [
      {
        ...
        label: 'Identify Mismatch',
        completed: true,
      },
      {
        step: 'contact_counterparty',
        label: 'Contact Counterparty',
        completed: false,
      },
      {
        step: 'create_matching_entry',
        label: 'Create Matching Entry',
        completed: false,
      },
      {
        step: 'confirm_elimination',
