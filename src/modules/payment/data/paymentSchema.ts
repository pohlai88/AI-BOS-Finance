...
export const PAYMENT_EXTENDED_FIELDS: MetadataField[] = [
  ...
  },
  {
    ...
    business_term: 'Docs Required',
    data_type: 'number',
    width: 60,
  },
  {
    technical_name: 'counterparty_entity',
    business_term: 'IC Counterparty',
    data_type: 'text',
    width: 140,
  },
]
...
export const MOCK_PAYMENTS: Payment[] = [
    ...
    manifests: [
      {
        type: 'invoice',
        ref_id: 'INV-2024-001',
        label: 'Invoice_INV-2024-001.pdf',
        file_size: '1.2 MB',
      },
      {
        type: 'po',
        ref_id: 'PO-4422',
        label: 'PurchaseOrder_4422.pdf',
        file_size: '850 KB',
      },
    ],
  },
  ...
  // --- CRITICAL: External, Very High Value, New Vendor ---
  {
    ...
    manifests: [
      {
        type: 'invoice',
        ref_id: 'CONS-Q1-FINAL',
        label: 'ConsultingQ1_Final.pdf',
        file_size: '2.1 MB',
      },
      {
        type: 'contract',
        ref_id: 'CON-110',
        label: 'ServiceAgreement_110.pdf',
        file_size: '1.8 MB',
      },
    ],
  },
    ...
    manifests: [
      {
        type: 'invoice',
        ref_id: 'LEG-RET-2024',
        label: 'Retainer_Q1_2024.pdf',
        file_size: '920 KB',
      },
      {
        type: 'contract',
        ref_id: 'CON-LEG-01',
        label: 'LegalServices_Agreement.pdf',
        file_size: '3.2 MB',
      },
    ],
  },
  ...
  // --- INTERCOMPANY: UNMATCHED (BLOCKED) ---
  {
    ...
    docs_required: 1,
    policy_violation:
      'Unilateral IC booking - no matching entry in counterparty',
    manifests: [],
  },
...
export function aggregateFunctionalClusters() {
  ...
  return clusters.map((cluster) => {
    const payments = MOCK_PAYMENTS.filter(
      (p) => p.functional_cluster === cluster && p.status === 'pending'
    )

    const anomalies = payments.filter((p) => p.risk_score > 50).length
    const hasUnmatchedIC = payments.some(
      (p) =>
        p.tx_type === 'intercompany' && p.elimination_status === 'unmatched'
    )
    ...
    return {
      ...
      status: hasUnmatchedIC
        ? 'blocked'
        : anomalies > 0
          ? 'anomalies'
          : 'clean',
      can_batch_approve:
        anomalies === 0 && !hasUnmatchedIC && cluster !== 'intercompany',
    }
  })
}
