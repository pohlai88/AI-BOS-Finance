...
export const ENTITY_OPTIONS: EntityOption[] = Object.values(TREASURY_DATA).map(t => ({
  id: t.entity_id,
  name: t.entity_name,
  code: t.entity_code,
...
export function getDistressedEntities(): TreasuryContext[] {
  return Object.values(TREASURY_DATA).filter(t => t.cash_status === 'critical');
}
...
export function getGroupTreasurySummary() {
  ...
  return {
    ...
    total_pending_payments: entities.reduce((sum, e) => sum + e.pending_payments_amount, 0),
    total_pending_count: entities.reduce((sum, e) => sum + e.pending_payments_count, 0),
    entities_critical: entities.filter(e => e.cash_status === 'critical').length,
 ...
 */
export function canAffordPayment(entityId: string, amount: number): {
  canAfford: boolean;
  remaining: number;
  warning?: string;
} {
