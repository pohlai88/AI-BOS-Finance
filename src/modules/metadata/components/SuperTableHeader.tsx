...
export const SuperTableHeader = <T,>({
  ...
  const getSortIcon = (colKey: string) => {
    if (sortConfig?.key !== colKey) {
      ...
      return (
        <ChevronsUpDown className="h-3.5 w-3.5 text-[#666] opacity-50 transition-opacity group-hover:opacity-100" />
      )
    }
    // Active sort state
    return sortConfig.direction === 'asc' ? (
      <ArrowUp className="h-3.5 w-3.5 text-[#28E7A2]" />
    ) : (
      <ArrowDown className="h-3.5 w-3.5 text-[#28E7A2]" />
    )
  }
  ...
  return (
    <thead
      className={cn(
        ...
        // Desktop styling
        'sticky top-0 z-10 border-b border-[#1F1F1F] bg-[#0A0A0A]',
        className
      )}
    >
      <tr>
        ...
        {enableSelection && (
          <th className="w-12 bg-[#0A0A0A] px-4 py-3 text-left">
            <input
              type="checkbox"
              className={cn(
                'h-4 w-4 rounded border-[#333] bg-[#111] text-[#28E7A2]',
                'focus:ring-1 focus:ring-[#28E7A2] focus:ring-offset-0',
          ...
          return (
            ...
            >
              <div className="flex flex-col gap-2">
                ...
                <div
                  className={cn(
                    'group flex select-none items-center gap-1.5',
                ...
                {col.filterable && (
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Filter..."
                      className={cn(
                        'block w-full rounded border bg-[#111] py-1.5 pl-7 pr-2',
                        'font-mono text-[11px] text-white placeholder:text-[#444]',
                    ...
                    {filters[colKey] && (
                      <button
                        className="absolute inset-y-0 right-0 flex items-center pr-2 text-[#666] transition-colors hover:text-[#28E7A2]"
                        onClick={(e) => {
                          e.stopPropagation()
