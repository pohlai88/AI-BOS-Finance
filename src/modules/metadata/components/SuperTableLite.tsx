...
export function SuperTableLite<T>({
  ...
  const filteredData = useMemo(() => {
    ...
    Object.entries(columnFilters).forEach(([key, filterValue]) => {
      if (filterValue) {
        result = result.filter((row) => {
          const value = String((row as Record<string, unknown>)[key] || '').toLowerCase();
  ...
  return (
    <div className={cn("flex flex-col bg-[#0A0A0A] border border-[#1F1F1F] rounded-lg overflow-hidden", className)}>
      
      {/* ================================================================ */}
      {/* HEADER BAR */}
      {/* ================================================================ */}
      <div className="flex flex-col md:flex-row md:items-center justify-between p-4 border-b border-[#1F1F1F] gap-4">
        
        {/* Left: Title + Stats */}
        <div className="flex items-center gap-3 flex-wrap">
          <div className="w-2 h-2 rounded-full bg-[#28E7A2] shadow-[0_0_8px_rgba(40,231,162,0.5)] animate-pulse" />
          <span className="text-[#CCC] text-[13px] font-medium tracking-wide font-mono">
            {title}
          </span>
          ...
          {enableGlobalSearch && (
            <div className="relative flex-1 md:flex-none">
              ...
              <input
                ...
                placeholder="Search..."
                className={cn(
                  "h-8 pl-9 pr-8 bg-[#050505] border border-[#1F1F1F] rounded",
                  "text-white text-[12px] font-mono",
                  "focus:outline-none focus:border-[#28E7A2]",
                  "placeholder:text-[#333] w-full md:w-48"
                )}
              />
              {globalFilter && (
                <button
