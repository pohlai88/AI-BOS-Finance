...
export const SuperTablePagination = ({
  ...
  const NavButton = ({
    ...
    <button
      className={cn(
        'rounded border border-[#222] bg-[#111] p-1.5',
        'transition-all duration-200',
        disabled
          ? 'cursor-not-allowed opacity-30'
          : 'hover:border-[#28E7A2]/30 hover:bg-[#1A1A1A] hover:text-[#28E7A2]'
      )}
      onClick={onClick}
      disabled={disabled}
      aria-label={label}
  ...
  return (
    <div
      className={cn(
        ...
        'border-t border-[#1F1F1F] bg-[#0A0A0A] px-4 py-3',
        className
      )}
    >
      ...
      <div className="flex w-full flex-col items-center gap-4 sm:w-auto sm:flex-row">
        ...
        <span className="font-mono text-[11px] text-[#666]">
          Showing <span className="font-medium text-[#28E7A2]">{startRow}</span>{' '}
          to <span className="font-medium text-[#28E7A2]">{endRow}</span> of{' '}
          <span className="font-medium text-white">{totalCount}</span> results
        </span>

        {/* Page Size Selector */}
        <div className="flex items-center gap-2">
          <span className="hidden font-mono text-[10px] uppercase tracking-wider text-[#555] sm:inline">
            Rows
          </span>
          <select
            value={pageSize}
            ...
            className={cn(
              ...
              'py-1.5 pl-2 pr-6 font-mono text-xs',
      ...
      <div className="flex items-center gap-1">
        ...
        {/* Page Indicator */}
        <div className="mx-1 rounded border border-[#222] bg-[#111] px-3 py-1">
