...
const INITIAL_COA: AccountNode[] = [
  {
    id: '1000', code: '1000', name: 'ASSETS', type: 'ASSET', level: 0, status: 'SYSTEM_LOCKED', expanded: true,
    children: [
      {
        id: '1100', code: '1100', name: 'Current Assets', type: 'ASSET', level: 1, status: 'SYSTEM_LOCKED', expanded: true,
        children: [
          { id: '1110', code: '1110', name: 'Cash & Equivalents', type: 'ASSET', level: 2, status: 'ACTIVE', balance: '$1,240,500.00' },
          { id: '1120', code: '1120', name: 'Accounts Receivable', type: 'ASSET', level: 2, status: 'ACTIVE', balance: '$342,100.00' },
          { id: '1130', code: '1130', name: 'Inventory', type: 'ASSET', level: 2, status: 'ACTIVE', balance: '$0.00' },
        ]
      },
      {
        id: '1200', code: '1200', name: 'Non-Current Assets', type: 'ASSET', level: 1, status: 'SYSTEM_LOCKED', expanded: false,
        children: [
          { id: '1210', code: '1210', name: 'Property, Plant & Equipment', type: 'ASSET', level: 2, status: 'ACTIVE', balance: '$4,500,000.00' },
          { id: '1220', code: '1220', name: 'Intangible Assets', type: 'ASSET', level: 2, status: 'ACTIVE', balance: '$150,000.00' },
        ]
      }
    ]
  },
  {
    id: '2000', code: '2000', name: 'LIABILITIES', type: 'LIABILITY', level: 0, status: 'SYSTEM_LOCKED', expanded: false,
    children: [
      {
        id: '2100', code: '2100', name: 'Current Liabilities', type: 'LIABILITY', level: 1, status: 'SYSTEM_LOCKED', children: [
          { id: '2110', code: '2110', name: 'Accounts Payable', type: 'LIABILITY', level: 2, status: 'ACTIVE', balance: '$120,400.00' },
        ]
      }
    ]
  },
  {
    id: '3000', code: '3000', name: 'EQUITY', type: 'EQUITY', level: 0, status: 'SYSTEM_LOCKED', expanded: false, children: []
  },
  {
    id: '4000', code: '4000', name: 'REVENUE', type: 'REVENUE', level: 0, status: 'SYSTEM_LOCKED', expanded: false, children: []
  },
  {
    id: '5000', code: '5000', name: 'EXPENSES', type: 'EXPENSE', level: 0, status: 'SYSTEM_LOCKED', expanded: false, children: []
  }
];
...
const TypeBadge = ({ type }: { type: AccountType }) => {
  ...
  return (
    <span className={`text-[9px] font-mono uppercase tracking-wider px-1.5 py-0.5 rounded border ${styles[type]}`}>
      {type}
    </span>
  );
};

const AccountRow = ({ node, level, onToggle }: { node: AccountNode, level: number, onToggle: (id: string) => void }) => {
  ...
  return (
    <div className="group relative">
      ...
      >
        ...
        {/* Code */}
        <span className={`font-mono text-xs mr-4 ${level === 0 ? 'font-bold text-white' : 'text-zinc-400'}`}>
          {node.code}
        </span>

        {/* Name */}
        <span className={`flex-1 text-sm ${level === 0 ? 'font-medium text-white' : 'text-zinc-300'}`}>
          {node.name}
        </span>
      ...
      <div className="px-6 py-8 md:px-12 md:py-12 max-w-[1400px] mx-auto">
        ...
        <div className="mt-8 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-2 flex-1 max-w-md">
            ...
            <Button variant="outline" className="border-[#1F1F1F] bg-black text-zinc-400 hover:text-white">
