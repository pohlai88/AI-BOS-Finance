...
const SCHEMA_SYSTEMS = [
  ...
  },
  {
    ...
    violations: [
      { field: 'Account.AnnualRevenue', issue: 'Type mismatch: Expected DECIMAL(18,2), found DECIMAL(20,4)', severity: 'medium', impactedSystems: 3, downstreamTables: 12 }
    ]
  },
  {
    id: 'snowflake',
    ...
    violations: [
      { field: 'FACT_SALES.REVENUE_AMT', issue: 'Canon violation: Precision changed from (18,2) to (18,4)', severity: 'critical', impactedSystems: 7, downstreamTables: 34 },
      { field: 'DIM_CUSTOMER.EMAIL', issue: 'Missing NOT NULL constraint', severity: 'high', impactedSystems: 2, downstreamTables: 8 }
    ]
  },
  {
    id: 'postgres',
...
const RECENT_EVENTS = [
  { time: '2m ago', system: 'SAP ERP', event: 'Schema validation passed', severity: 'info', table: 'VBAK' },
  { time: '5m ago', system: 'Salesforce', event: 'Type drift detected', severity: 'warning', table: 'Account.AnnualRevenue' },
  { time: '12m ago', system: 'Snowflake DW', event: 'Canon violation detected', severity: 'critical', table: 'FACT_SALES.REVENUE_AMT' },
  { time: '18m ago', system: 'PostgreSQL', event: 'Schema validation passed', severity: 'info', table: 'orders' },
  { time: '23m ago', system: 'Snowflake DW', event: 'Constraint violation detected', severity: 'high', table: 'DIM_CUSTOMER.EMAIL' },
];

export function MetadataArchitecturePage() {
  const [selectedSystem, setSelectedSystem] = useState<string | null>(null);
  const [selectedViolation, setSelectedViolation] = useState<{system: string, field: string} | null>(null);
  const selected = SCHEMA_SYSTEMS.find(s => s.id === selectedSystem);

  const violation = selectedViolation 
    ? SCHEMA_SYSTEMS
        .find(s => s.id === selectedViolation.system)
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
          ...
          <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Schemas</div>
            </div>
            <div className="text-white font-mono text-2xl">{LIVE_METRICS.totalSchemas.toLocaleString()}</div>
          </div>
          ...
          <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Validated</div>
            </div>
            <div className="text-[#28E7A2] font-mono text-2xl">{LIVE_METRICS.validated.toLocaleString()}</div>
          </div>
          ...
          <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Violations</div>
            </div>
          ...
          <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Systems</div>
            </div>
          ...
          <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Compliance</div>
            </div>
          ...
          <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Uptime</div>
            </div>
          ...
          <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Last Event</div>
            </div>
            ...
            <div className="border-b border-[#1F1F1F] p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-white font-mono uppercase tracking-wider text-sm">System Bindings</h3>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#28E7A2] animate-pulse" />
                  <span className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Live</span>
                </div>
              </div>
            </div>
              ...
              {SCHEMA_SYSTEMS.map(system => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      {system.status === 'healthy' && <CheckCircle className="w-4 h-4 text-[#28E7A2] flex-shrink-0" />}
                      {system.status === 'warning' && <AlertTriangle className="w-4 h-4 text-yellow-500 flex-shrink-0" />}
                      {system.status === 'critical' && <AlertCircle className="w-4 h-4 text-red-500 flex-shrink-0" />}
                      <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <ChevronRight className={`w-4 h-4 text-[#666] transition-transform ${selectedSystem === system.id ? 'rotate-90' : ''}`} />
                    </div>
                  </div>
                        ...
                        >
                          <div className="flex items-start gap-2 mb-1">
                            ...
                            <div className="flex-1">
                              <div className="text-xs font-mono text-white mb-1">{violation.field}</div>
                              <div className="text-[11px] text-[#888] leading-relaxed">{violation.issue}</div>
                          ...
                          <div className="flex items-center gap-2">
                            <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono uppercase tracking-wider ${
                              violation.severity === 'critical' 
          ...
          <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded overflow-hidden">
            <div className="border-b border-[#1F1F1F] p-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-white font-mono uppercase tracking-wider text-sm">Canonical Data Flow</h3>
              </div>
          ...
          <div className="border-b border-[#1F1F1F] p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-white font-mono uppercase tracking-wider text-sm">Event Stream</h3>
              </div>
              <div className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Last 30 minutes</div>
            </div>
          </div>
          ...
          >
            ...
            >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-black/50 border border-[#1F1F1F] rounded p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Impacted Systems</div>
                    </div>
                  ...
                  <div className="bg-black/50 border border-[#1F1F1F] rounded p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Downstream Tables</div>
                    </div>
                    <div className="text-white font-mono text-3xl">{violation.downstreamTables}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 overflow-y-auto max-h-[400px]">
                <h4 className="text-white font-mono uppercase tracking-wider text-sm mb-4">Downstream Dependencies</h4>
                
                <div className="space-y-3">
                  {/* Example downstream dependencies */}
                  {[
                    { system: 'Tableau Analytics', tables: ['Revenue Dashboard', 'Executive Summary', 'Sales Forecast'] },
                ...
                <div className="mt-6 p-4 bg-[#28E7A2]/5 border border-[#28E7A2]/20 rounded">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <div className="text-xs font-mono text-[#28E7A2] uppercase tracking-wider mb-1">Recommended Action</div>
                      <div className="text-xs text-[#888] leading-relaxed">
                        Update {violation.field} in source system to match Canon specification, or update Canon if the business logic has changed. 
                        Run validation after changes to verify propagation across all {violation.downstreamTables} dependent tables.
                      </div>
                    </div>
                  </div>
                </div>
