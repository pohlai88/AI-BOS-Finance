...
export default function RadarDisplay({ config, points, onPointClick }: RadarDisplayProps) {
  ...
  useEffect(() => {
    ...
    const drawPoints = (currentSweepAngle: number) => {
      points.forEach(point => {
        ...
        const normalizedSweepAngle = ((currentSweepAngle * 180 / Math.PI) + 90) % 360;
        ...
        if (opacity > 0) {
          ...
          if (isUnderSweep) {
            const shineGradient = ctx.createRadialGradient(x, y, 0, x, y, (point.size || 6) * 8);
          ...
          // Draw glow
          const glowGradient = ctx.createRadialGradient(x, y, 0, x, y, (point.size || 6) * 3);
          glowGradient.addColorStop(0, (point.color || '#95C7FD') + Math.floor(opacity * 180).toString(16).padStart(2, '0'));
          glowGradient.addColorStop(0.5, (point.color || '#95C7FD') + Math.floor(opacity * 80).toString(16).padStart(2, '0'));
