          ...
          <div className="bg-[#0A0A0A] border border-[#1F1F1F] relative">
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-4 gap-6 mb-6 pb-6 border-b border-[#1F1F1F]">
                ...
                {/* Version */}
                <div>
                  <div className="font-mono text-[9px] text-[#666] uppercase tracking-wider mb-2">Version</div>
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 border border-[#28E7A2]/30 bg-[#28E7A2]/5">
                    <span className="font-mono text-sm text-[#28E7A2] tracking-wider">{data.version}</span>
                  </div>
                </div>

                {/* Status */}
                <div>
                  <div className="font-mono text-[9px] text-[#666] uppercase tracking-wider mb-2">Status</div>
                  <div className="flex items-center gap-2">
                    <div 
                      className="w-2 h-2 rounded-full"
                      style={{ 
                        backgroundColor: getStatusColor(data.status),
                        animation: (data.status === 'VERIFIED' || data.status === 'ACTIVE') ? 'pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite' : 'none'
                      }}
                    />
                    <span className="font-mono text-sm tracking-wider" style={{ color: getStatusColor(data.status) }}>
                      {data.status}
                    </span>
                  </div>
                </div>
                ...
                {/* Last Updated */}
                <div>
                  <div className="font-mono text-[9px] text-[#666] uppercase tracking-wider mb-2">Last Updated</div>
                  <div className="font-mono text-[11px] text-[#999]">{formatTimestamp(data.lastUpdated)}</div>
                </div>

                {/* Classification */}
                <div>
                  <div className="font-mono text-[9px] text-[#666] uppercase tracking-wider mb-2">Classification</div>
                  <div className="font-mono text-[11px] text-[#999] uppercase tracking-wider">{data.classification}</div>
                </div>
              ...
              <div className="mb-6 pb-6 border-b border-[#1F1F1F]">
                <div className="font-mono text-[9px] text-[#666] uppercase tracking-wider mb-2">Validator</div>
                <div className="font-mono text-[11px] text-[#28E7A2] tracking-wider">{data.validator}</div>
              </div>

              {/* Recent Changes */}
              <div>
                <div className="font-mono text-[10px] text-white uppercase tracking-wider mb-4">Recent Changes</div>
                    ...
                    <div key={index} className="flex items-start gap-3 group">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          <span className="font-mono text-[10px] text-[#666]">{formatTimestamp(entry.timestamp)}</span>
                          {entry.validator && (
                            <>
                              <div className="w-[1px] h-2 bg-[#333]" />
                              <span className="font-mono text-[9px] text-[#555] uppercase tracking-wider">{entry.validator}</span>
                            </>
                          )}
                        </div>
                        <p className="font-mono text-[11px] text-[#999] leading-relaxed">{entry.change}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="pointer-events-auto w-full max-w-3xl max-h-[80vh] bg-[#000000] border-2 border-[#28E7A2] relative overflow-hidden">
              ...
              <div className="relative border-b border-[#1F1F1F]">
                ...
                <div className="px-8 py-6 flex items-center justify-between">
                  <div>
                    <h2 className="font-mono text-lg text-white tracking-wide mb-1">FULL AUDIT TRAIL</h2>
              ...
              <div className="relative overflow-y-auto max-h-[calc(80vh-120px)] px-8 py-6">
                <div className="space-y-3">
                  {data.fullHistory.map((entry, index) => (
                    <div key={index} className="flex items-start gap-3 pb-3 border-b border-[#1F1F1F] last:border-0">
                      <div className="flex-shrink-0 mt-1">
                        <div className="w-2 h-2 border border-[#28E7A2] rotate-45" />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          <span className="font-mono text-[10px] text-[#28E7A2]">{formatTimestamp(entry.timestamp)}</span>
                          {entry.validator && (
                            <>
                              <div className="w-[1px] h-2 bg-[#333]" />
                              <span className="font-mono text-[9px] text-[#666] uppercase tracking-wider">{entry.validator}</span>
                            </>
                          )}
                        </div>
                        <p className="font-mono text-[11px] text-[#999] leading-relaxed">{entry.change}</p>
