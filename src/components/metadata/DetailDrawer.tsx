...
import { 
  X, Maximize2, Copy, Check, FileJson, FileSpreadsheet, 
  Database, ArrowRight, ExternalLink, Share2, Shield, 
  Clock, User, Layers, FileText, Settings2, RotateCcw,
  GripVertical, Plus, Trash2, Save
} from 'lucide-react';
...
const DEFAULT_LAYOUT: SectionSchema[] = [
  {
    ...
    fields: [
      ...
      { id: 'biz_name', label: 'Business Name', key: 'business_term', type: 'row', span: 2, isVisible: true },
      { id: 'tech_name', label: 'Technical Name', key: 'technical_name', type: 'row', span: 2, isVisible: true },
  ...
  },
  {
    ...
    title: 'Definitions',
    fields: [
      { id: 'short_def', label: 'Short Definition', key: 'definition_full', type: 'card', span: 1, isVisible: true, transform: (r) => r.definition_full.split('.')[0] + '.' },
      { id: 'full_def', label: 'Full Definition', key: 'definition_full', type: 'card', span: 1, isVisible: true }
    ]
  },
  {
    id: 'usage',
    ...
    fields: [
      { id: 'examples', label: 'Example Values', key: 'example_values', type: 'tags', span: 1, isVisible: true },
      { id: 'usecases', label: 'Use Cases', key: 'use_cases', type: 'list', span: 1, isVisible: true, transform: () => ["Board pack reporting", "Q4 Audit Trail"] }
    ]
  },
  {
    id: 'source',
    ...
    fields: [
      { id: 'sot', label: 'Source of Truth', key: 'source_of_truth', type: 'row', span: 2, isVisible: true },
      { id: 'domain', label: 'Domain', key: 'domain', type: 'row', span: 2, isVisible: true },
      { id: 'conn', label: 'Connection Path', key: 'connection', type: 'row', span: 2, isVisible: true, transform: (r) => `jdbc://${r.domain}.db/${r.technical_name}` }
    ]
  },
  {
    id: 'specs',
    ...
    fields: [
      { id: 'biz_type', label: 'Business Type', key: 'data_type_biz', type: 'card', span: 1, isVisible: true },
      { id: 'tech_type', label: 'Technical Type', key: 'data_type_tech', type: 'card', span: 1, isVisible: true }
    ]
  },
  {
    id: 'gov',
    ...
    fields: [
      { id: 'risk', label: 'Risk Impact', key: 'errors_if_wrong', type: 'risk-alert', span: 2, isVisible: true, transform: (r) => r.errors_if_wrong || "Potential revenue misstatement and audit failure." }
    ]
  }
];
...
const CopyBtn = ({ text }: { text: string }) => {
  ...
  return (
    <button onClick={handleCopy} className={clsx("p-1.5 rounded transition-all", copied ? "text-[#28E7A2] bg-[#28E7A2]/10" : "text-[#666] hover:text-white hover:bg-[#222]")}>
...
const WidgetCard = ({ label, value }: { label: string; value: string }) => (
  <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded p-3 h-full relative group hover:border-[#333] transition-colors flex flex-col">
    <div className="flex justify-between items-start mb-2">
      <span className="text-[10px] font-mono text-[#666] uppercase tracking-wider">{label}</span>
      <div className="opacity-0 group-hover:opacity-100 transition-opacity"><CopyBtn text={value} /></div>
    </div>
...
const WidgetTags = ({ label, values }: { label: string; values: string[] }) => (
  <div className={clsx("flex flex-col gap-2 h-full justify-center", values.length === 0 && "hidden")}>
    {label !== 'Tags' && <span className="text-[10px] font-mono text-[#666] uppercase tracking-wider">{label}</span>}
    <div className="flex flex-wrap gap-2">
      {values.map((v, i) => (
         <span key={i} className="inline-block px-1.5 py-0.5 bg-[#1F1F1F] border border-[#333] rounded text-[#28E7A2] font-mono text-[10px]">{v}</span>
      ))}
    </div>
  </div>
);
...
const WidgetList = ({ label, items }: { label: string; items: string[] }) => (
  <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded p-3 h-full">
    <span className="text-[10px] font-mono text-[#666] uppercase tracking-wider block mb-2">{label}</span>
...
const WidgetRisk = ({ label, value }: { label: string; value: string }) => (
  <div className="bg-[#2F1515] border border-[#491C1C] rounded p-3 flex gap-3 items-start h-full">
    ...
    <div>
      <div className="text-[10px] font-mono text-[#FF4D4D] uppercase tracking-wider mb-1">{label}</div>
...
const SortableItem = ({ id, field, children, isEditing, onRemove }: { id: string, field: FieldSchema, children: React.ReactNode, isEditing: boolean, onRemove: () => void }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    >
      ...
      {isEditing && (
        <div className="absolute -inset-1 border-2 border-[#28E7A2] border-dashed rounded bg-[#28E7A2]/5 z-50 flex items-start justify-between p-1 pointer-events-none">
             <div {...attributes} {...listeners} className="pointer-events-auto cursor-grab active:cursor-grabbing p-1 bg-[#050505] rounded border border-[#28E7A2] hover:bg-[#28E7A2] hover:text-black transition-colors">
  ...
  return (
    <div className="space-y-6">
      {layout.map((section, sectionIndex) => (
        <div key={section.id} className={clsx("transition-all duration-300", isEditing && "p-4 border border-[#333] border-dashed rounded-lg bg-[#0A0A0A]")}>
          ...
          >
            ...
            >
                <div className="grid grid-cols-2 gap-4">
                    {section.fields.filter(f => f.isVisible !== false).map((field) => {
                        let rawValue: any = field.transform ? field.transform(record) : record[field.key as keyof MetadataRecord];
                        ...
                        return (
                            ...
                            >
                                {field.type === 'row' && <WidgetRow label={field.label} value={String(rawValue)} />}
                                {field.type === 'card' && <WidgetCard label={field.label} value={String(rawValue)} />}
                                {field.type === 'tags' && <WidgetTags label={field.label} values={Array.isArray(rawValue) ? rawValue : [String(rawValue)]} />}
                                {field.type === 'list' && <WidgetList label={field.label} items={Array.isArray(rawValue) ? rawValue : [String(rawValue)]} />}
                                {field.type === 'risk-alert' && <WidgetRisk label={field.label} value={String(rawValue)} />}
                            </SortableItem>
                        );
                    })}
                </div>
...
const LineageMap = ({ record }: { record: MetadataRecord }) => (
    <div className="relative h-32 bg-[#080808] border border-[#1F1F1F] rounded overflow-hidden mb-4 group">
      <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(#333 1px, transparent 1px)', backgroundSize: '16px 16px' }} />
      <div className="absolute inset-0 flex items-center justify-center px-8 gap-4">
        <div className="flex flex-col items-center gap-2 opacity-60">
          <div className="p-2 rounded bg-[#111] border border-[#333]"><Database className="w-4 h-4 text-[#666]" /></div>
          <span className="text-[9px] font-mono text-[#666]">SRC</span>
        </div>
        <div className="h-px w-12 bg-[#333] relative"><ArrowRight className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-3 h-3 text-[#333]" /></div>
        <div className="flex flex-col items-center gap-2">
          <div className="p-2.5 rounded bg-[#132F2B] border border-[#28E7A2] shadow-[0_0_15px_rgba(40,231,162,0.15)]"><Layers className="w-5 h-5 text-[#28E7A2]" /></div>
          <span className="text-[9px] font-mono text-[#28E7A2] font-bold">THIS</span>
        </div>
        <div className="h-px w-12 bg-[#333] relative"><ArrowRight className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-3 h-3 text-[#333]" /></div>
        <div className="flex flex-col items-center gap-2 opacity-60">
          <div className="p-2 rounded bg-[#111] border border-[#333]"><Share2 className="w-4 h-4 text-[#666]" /></div>
          <span className="text-[9px] font-mono text-[#666]">{record.downstream_use.length} USERS</span>
        </div>
      </div>
...
const ExportButton = ({ 
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        <Icon className={clsx("w-3.5 h-3.5", isOpen ? "stroke-black" : "group-hover:stroke-black")} /> 
        {label}
      </button>
      ...
      {isOpen && (
        <div className="absolute bottom-full right-0 mb-2 w-56 bg-[#0A0A0A] border border-[#333] rounded shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
          <div className="px-3 py-2 border-b border-[#1F1F1F] bg-[#111]">
            <span className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Select {label} Scope</span>
          </div>
          
          <div className="p-1">
            {/* OPTION 1: CURRENT VIEW */}
            <button 
              onClick={() => { handleDownload(format, 'selected', record, layout); setIsOpen(false); }}
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-xs text-[#CCC] group-hover:text-white font-mono">Current View</span>
            ...
            {/* OPTION 2: FULL DATA */}
            <button 
              onClick={() => { handleDownload(format, 'full', record, layout); setIsOpen(false); }}
              className="w-full text-left px-3 py-2 hover:bg-[#1A1A1A] rounded group flex items-start gap-3 transition-colors"
            >
              <Database className="w-4 h-4 text-[#666] group-hover:text-[#28E7A2] mt-0.5 shrink-0" />
              <div className="flex flex-col">
                <span className="text-xs text-[#CCC] group-hover:text-white font-mono">Full Fact Sheet</span>
                <span className="text-[9px] text-[#666] leading-tight">All raw database fields</span>
              </div>
            </button>
          </div>
        </div>
          ...
          >
            ...
            <div className="flex-none px-6 py-4 border-b border-[#1F1F1F] bg-[#050505] flex items-center justify-between z-10">
              <div className="flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full ${record.canon_status === 'LOCKED' ? 'bg-[#28E7A2]' : 'bg-yellow-500'} animate-pulse`} />
              ...
              <div className="flex items-center gap-2">
                 ...
                 {isEditing ? (
                     <>
                        <button onClick={handleReset} className="flex items-center gap-2 px-3 py-1.5 bg-red-900/20 border border-red-900/50 text-red-500 hover:bg-red-900/40 rounded text-[10px] font-mono uppercase transition-colors">
                            <RotateCcw className="w-3 h-3" /> Reset
                        </button>
                        <button onClick={handleSave} className="flex items-center gap-2 px-3 py-1.5 bg-[#28E7A2] border border-[#28E7A2] text-black hover:bg-[#28E7A2]/90 rounded text-[10px] font-mono font-bold uppercase transition-colors shadow-[0_0_10px_rgba(40,231,162,0.3)]">
                            <Save className="w-3 h-3" /> Save View
                        </button>
                     </>
                 ) : (
                    <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 px-4 py-2 bg-[#132F2B] border border-[#28E7A2] text-[#28E7A2] hover:bg-[#28E7A2] hover:text-black rounded text-[11px] font-mono font-bold uppercase transition-all shadow-[0_0_15px_rgba(40,231,162,0.2)] hover:shadow-[0_0_25px_rgba(40,231,162,0.4)] group">
                        <Settings2 className="w-4 h-4 group-hover:rotate-90 transition-transform duration-300" /> Customize
                    </button>
                 )}
            ...
            {/* CONTENT */}
            <div className={clsx("flex-1 overflow-y-auto custom-scrollbar p-8", isEditing && "bg-[#080808]")}>
              {/* STATIC HERO SECTION (Lineage + Title) */}
              <div className={clsx("transition-opacity duration-300", isEditing && "opacity-50 pointer-events-none")}>
                 <LineageMap record={record} />

                 <div className="mb-6 flex justify-end">
                    <Link to={`/metadata/${record.dict_id}`} className="flex items-center gap-2 text-xs font-mono text-[#28E7A2] hover:underline uppercase tracking-wide">
                 ...
                 <div className="mb-6 border-l-2 border-[#28E7A2] pl-4">
                    <div className="flex items-center gap-2 mb-1">
                       <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-[#1F1F1F] text-[#888]">{record.dict_id}</span>
                       <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-[#132F2B] text-[#28E7A2] border border-[#1D4436]">{record.canon_status}</span>
                    </div>
              ...
              {!isEditing && (
                <div className="mt-8 mb-4 flex justify-center border-t border-[#1F1F1F] pt-6">
                  <Link to={`/metadata/${record.dict_id}`} className="flex items-center gap-2 px-6 py-3 bg-[#111] hover:bg-[#222] border border-[#333] rounded text-xs font-mono uppercase tracking-widest text-white transition-all">
            ...
            {!isEditing && (
                <div className="flex-none p-4 border-t border-[#1F1F1F] bg-[#0A0A0A] flex items-center justify-between">
                    <span className="text-[11px] font-mono font-bold text-[#28E7A2] uppercase tracking-[0.12em]">Take this metadata to your tools</span>
                    <div className="flex items-center gap-2">
