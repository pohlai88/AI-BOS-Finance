...
const MorphologyNode = ({ angle, emoji, label, sublabel, examples, color, tier, isMerged }: MorphologyNodeProps) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="font-mono text-[8px] tracking-wider uppercase font-semibold" style={{ color: colors.text }}>
          {label}
        </div>
      </motion.div>
    </motion.div>
...
const Tether = ({ angle, color, isMerged }: { angle: number; color: string; isMerged: boolean }) => {
  const colorMap: Record<string, string> = { yellow: '#FFD600', green: '#28E7A2', white: '#FFFFFF' };
          ...
          >
            ...
            <AnimatePresence mode="wait">
              {isMerged ? (
                ...
                >
                  <span className="font-mono text-[6px] text-[#FFD600] uppercase font-bold tracking-widest bg-black/50 px-1">Immutable</span>
                  <span className="font-mono text-[5px] text-white uppercase tracking-widest">Metadata</span>
                </motion.div>
              ) : (
                <motion.div 
                  key="base-label"
                  ...
                  className="absolute bottom-4 flex flex-col items-center"
                >
                  <span className="font-mono text-[7px] text-[#888] uppercase tracking-widest">Base Canon</span>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
          ...
          <AnimatePresence>
            {isMerged && (
              <>
                ...
                >
                  <div className="text-[9px] text-[#666] uppercase tracking-[0.15em] mb-1">Morphology</div>
                ...
                <motion.div 
                  ...
                  transition={{ duration: 0.6, delay: 0.3 }}
                >
                  <div className="text-[9px] text-[#666] uppercase tracking-[0.15em] mb-1">Metadata</div>
            ...
            {isMerged && (
              ...
              >
                <div className="flex items-center gap-2 bg-[#FFD600]/10 border border-[#FFD600] px-3 py-1 rounded-full">
                  ...
                  <span className="font-mono text-[8px] text-[#FFD600] tracking-[0.15em] uppercase">Verified</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
        <div className="flex items-center gap-4 px-6 py-3 bg-[#0A0A0A]/90 backdrop-blur border border-[#333] rounded-full shadow-2xl">
           ...
           >
             ...
             <span className={`font-mono text-[10px] tracking-wider transition-colors duration-500 ${isMerged ? 'text-[#FFD600]' : 'text-white'}`}>
