    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[20vh]">
        ...
        <motion.div
          ...
          className="relative z-10 w-full max-w-2xl overflow-hidden border-2 border-[#28E7A2] bg-[#000000]"
        >
          {/* Top Glow */}
          ...
          <div className="relative border-b border-[#1F1F1F]">
            <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#666]" />
            <input
              ref={inputRef}
              type="text"
              value={search}
              ...
              className="w-full bg-transparent py-4 pl-12 pr-4 font-mono text-sm text-white outline-none placeholder:text-[#666]"
            />
            <div className="absolute right-4 top-1/2 -translate-y-1/2 font-mono text-[10px] uppercase tracking-wider text-[#444]">
              ESC to close
            </div>
          </div>
          ...
          <div className="max-h-[400px] overflow-y-auto">
            {filteredCommands.length === 0 ? (
              <div className="py-12 text-center">
                <div className="mb-2 font-mono text-sm text-[#666]">
                  No results found
                </div>
              ...
              filteredCommands.map((cmd, index) => {
                ...
                return (
                  ...
                  >
                    {/* Icon */}
                    <div
                      className={`flex h-8 w-8 shrink-0 items-center justify-center border ${isSelected ? 'border-[#28E7A2]' : 'border-[#333]'} `}
                    >
                      <Icon
                        className={`h-4 w-4 ${isSelected ? 'text-[#28E7A2]' : 'text-[#666]'}`}
                      />
                    </div>
                    ...
                    <div className="flex-1 text-left">
                      <div
                        className={`mb-0.5 font-mono text-sm ${isSelected ? 'text-white' : 'text-[#CCC]'}`}
                      >
                        {cmd.title}
                      </div>
                      {cmd.subtitle && (
                    ...
                    {/* Arrow */}
                    {isSelected && (
                      <ArrowRight className="h-4 w-4 text-[#28E7A2]" />
                    )}
                  </button>
                )
              })
          ...
          {/* Footer */}
          <div className="flex items-center justify-between border-t border-[#1F1F1F] bg-[#0A0A0A] px-4 py-3">
            <div className="font-mono text-[9px] uppercase tracking-wider text-[#444]">
              {filteredCommands.length} result
              {filteredCommands.length !== 1 ? 's' : ''}
            </div>
            <div className="flex items-center gap-4 font-mono text-[9px] uppercase tracking-wider text-[#444]">
              <span>
