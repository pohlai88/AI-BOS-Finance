...
export const CrystallizationVisual = () => (
  <div className="w-full h-full flex items-center justify-center relative">
    ...
    <div className="flex gap-4">
      {[1, 2, 3].map((i) => (
        ...
        >
          ...
          {i === 3 && (
            <motion.div
              className="absolute inset-0 bg-nexus-green/20 border border-nexus-green rounded"
    ...
    <motion.div
      className="absolute right-6 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-nexus-green/10 border border-nexus-green/30 flex items-center justify-center"
      animate={{ scale: [1, 1.1, 1] }}
      transition={{ duration: 2, repeat: Infinity }}
    >
      <div className="w-3 h-3 border-2 border-nexus-green rounded-sm" />
    </motion.div>
  </div>
);
...
export const InterrogationVisual = () => (
  <div className="w-full h-full flex items-center justify-center relative overflow-hidden">
    ...
    <div className="absolute w-16 h-16 border border-nexus-green/30 rounded-full bg-nexus-green/5" />
    
    {/* Center Pulse */}
    <motion.div
      className="absolute w-4 h-4 bg-nexus-green rounded-full"
...
export const GovernanceVisual = () => (
  <div className="w-full h-full p-6 flex flex-col gap-3 relative">
    ...
    {[
      ...
      >
        <motion.div
          className={`w-3 h-3 rounded-full ${line.status === 'fail' ? 'bg-red-500/50' : 'bg-nexus-green/50'}`}
