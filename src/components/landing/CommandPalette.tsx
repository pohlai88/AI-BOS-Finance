...
import { 
  Search, Command, FileText, Settings, ShieldCheck, 
  ArrowRight, Calculator, Hash, Zap, X 
} from 'lucide-react';

// --- MOCK DATA FOR THE TERMINAL ---
const COMMANDS = [
  { id: 'cmd_1', type: 'ACTION', label: 'Initiate Forensic Scan', icon: <Zap className="w-4 h-4" />, shortcut: 'S' },
  { id: 'cmd_2', type: 'ACTION', label: 'Crystallize Current Period', icon: <ShieldCheck className="w-4 h-4" />, shortcut: 'C' },
...
export const CommandPalette = ({ isOpen: controlledIsOpen, onClose: controlledOnClose }: CommandPaletteProps = {}) => {
            ...
            >
              ...
              <div className="px-4 py-2 bg-zinc-950 border-t border-white/5 flex justify-between items-center text-[10px] text-zinc-500 font-mono">
                 <div className="flex gap-4">
                    <span><span className="text-zinc-300">↑↓</span> to navigate</span>
                    <span><span className="text-zinc-300">↵</span> to execute</span>
                 </div>
  ...
  return (
    ...
    `}>
       <div className="flex items-center gap-3">
          <div className={`p-1.5 rounded-md ${selected ? 'text-emerald-400' : 'text-zinc-500 group-hover:text-zinc-300'}`}>
             {item.type === 'HASH' ? <Hash className="w-4 h-4" /> : item.icon}
          </div>
          <div className="flex flex-col">
             <span className={`text-sm ${selected ? 'text-white' : 'text-zinc-300'}`}>{item.label}</span>
