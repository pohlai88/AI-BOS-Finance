      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full border border-zinc-800 bg-zinc-900/80 backdrop-blur-sm">
            <div className={`w-2 h-2 rounded-full ${isHovered ? 'bg-emerald-500' : 'bg-red-500 animate-pulse'}`} />
            <span className="text-xs font-mono text-zinc-400 uppercase tracking-widest">
              Unmonitored Ledger Risk: <span className={isHovered ? "text-emerald-400" : "text-white"}>${riskCost.toLocaleString()}</span>
            </span>
          </div>
...
const Metric = ({ label, value, sub }: { label: string, value: string, sub: string }) => (
  <div className="bg-zinc-950 p-6 flex flex-col items-center justify-center group hover:bg-zinc-900 transition-colors">
     ...
     <span className="text-xl font-mono text-white font-bold group-hover:text-emerald-400 transition-colors">{value}</span>
