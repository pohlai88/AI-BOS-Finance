...
import { FileSpreadsheet, PackageCheck, FileText, ArrowRight, ShieldCheck, Lock, Link2 } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        transition={{ duration: isLocked ? 0.8 : 40, ease: isLocked ? "backOut" : "linear", repeat: isLocked ? 0 : Infinity }} // Faster snap
        style={{ animationPlayState: !isLocked && animationState === 'paused' ? 'paused' : 'running' }}
      >
...
const Tether = ({ angle, color, active }: {
  angle: number;
  color: string;
  active: boolean;
}) => {
   const colors: Record<string, string> = { emerald: '#10b981', amber: '#f59e0b', purple: '#a855f7' };
...
const OrbitNode = ({ angle, id, label, sub, icon, color, setHovered, isHovered, isLocked, data }: {
  angle: number;
  id: string;
  label: string;
  sub: string;
  ...
  return (
    ...
    >
      <motion.div 
        className="relative"
        animate={isLocked ? { rotate: 0 } : { rotate: -360 }} 
        transition={{ duration: isLocked ? 0.8 : 40, ease: isLocked ? "backOut" : "linear", repeat: isLocked ? 0 : Infinity }}
      >
        <motion.div
        ...
        >
          {/* Header */}
          <div className={`p-4 flex items-center gap-3 border-b ${isHovered || isLocked ? 'border-white/10' : 'border-transparent'}`}>
             <div className={`p-2 rounded-lg ${isHovered || isLocked ? 'bg-white/5' : ''}`}>
               {icon}
             </div>
             <div>
               <div className={`text-xs font-bold tracking-widest uppercase ${isHovered || isLocked ? 'text-white' : ''}`}>{label}</div>
              ...
              >
                <div className="p-4 space-y-4">
                  ...
                  <div className="space-y-3 text-xs">
                    <Row label="ANCESTRY" value={data.past.val} icon={<Link2 className="w-3 h-3 text-zinc-500" />} />
                    <Row label="STATUS" value={data.current.val} highlight={data.current.val.includes("OPEN") || data.current.val.includes("BLOCKED")} icon={<ArrowRight className="w-3 h-3 text-white" />} />
                  </div>
                  ...
                  <div className="pt-3 border-t border-white/5 mt-2 flex justify-between items-center">
                    ...
                    {isLocked && <div className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded uppercase tracking-wider font-bold">Verified</div>}
                  </div>
                </div>
              </motion.div>
            )}
...
const Row = ({ label, value, highlight, icon }: {
  ...
  <div className="flex items-start gap-2">
    ...
    <div className="flex-1">
      ...
      <div className={`font-mono text-xs ${highlight ? 'text-white font-bold' : 'text-zinc-400'}`}>{value}</div>
    </div>
  </div>
);