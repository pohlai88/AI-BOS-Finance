    ...
    <div className="border border-[#1F1F1F] bg-[#0A0A0A] overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
      ...
      <div className="flex items-center justify-between px-8 py-4 border-b border-[#1F1F1F] bg-[#111]">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-mono text-[#666] uppercase tracking-widest">DEEP DIVE SCAN // {module.id}</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[400px]">
        ...
        <div className="col-span-12 lg:col-span-4 p-8 border-b lg:border-b-0 lg:border-r border-[#1F1F1F] bg-[#0F0F0F]">
          ...
          <div className="space-y-6">
            <ScoreBar label="IFRS Alignment" score={module.subScores.ifrs} icon={<FileText className="w-3 h-3" />} />
            <ScoreBar label="Tax Consistency" score={module.subScores.tax} icon={<AlertCircle className="w-3 h-3" />} />
            <ScoreBar label="Control Evidence" score={module.subScores.controls} icon={<Shield className="w-3 h-3" />} />
          </div>

          <div className="mt-12 p-4 border border-[#333] bg-[#0A0A0A] rounded-sm">
            <h4 className="text-[10px] text-[#666] uppercase tracking-widest mb-2">Forensic Summary</h4>
            <p className="text-xs text-[#BBB] leading-relaxed">
              This module is currently <span className="text-white font-medium">{module.status}</span>. 
              Review the issues on the right. Critical findings in IFRS alignment require immediate remediation before next audit cycle.
            </p>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-8 p-0">
          <div className="flex items-center gap-4 px-8 py-4 border-b border-[#1F1F1F]">
            <span className="text-xs font-mono text-[#666] uppercase tracking-widest">Detected Findings ({module.issues.length})</span>
          </div>
    ...
    <div className="p-6 hover:bg-[#111] transition-colors group">
      <div className="flex items-start justify-between gap-4 mb-2">
        ...
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
           <button className="p-1.5 border border-[#333] hover:border-[#666] text-[#666] hover:text-[#EEE] rounded-sm" title="View in Risk Radar">
             <Activity className="w-3 h-3" />
           </button>
           <button className="p-1.5 border border-[#333] hover:border-[#666] text-[#666] hover:text-[#EEE] rounded-sm" title="View Canon">
...
function Badge({ type, outline = false }: { type: string, outline?: boolean }) {
  ...
  return (
    <span className={`px-1.5 py-0.5 text-[9px] font-mono uppercase tracking-wider rounded-sm ${colors}`}>
      {type}
    </span>
  );
}
