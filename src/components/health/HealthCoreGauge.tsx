...
export function HealthCoreGauge({ data, color = "#28E7A2" }: HealthCoreGaugeProps) {
  ...
  const formattedData = data.map((d, i) => {
    ...
    if (d.A < 60) ringColor = '#EF4444'; // Critical (Red)
    ...
    <div className="w-full h-[320px] relative flex items-center justify-center bg-[#050505] overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <svg className="absolute inset-0 w-full h-full opacity-30">
          <circle cx="50%" cy="50%" r="48%" fill="none" stroke="#28E7A2" strokeWidth="1" strokeDasharray="4 4" />
      ...
      <div className="w-full h-full relative z-10">
        <ResponsiveContainer width="100%" height="100%" minHeight={300} minWidth={100} debounce={200}>
          <RadialBarChart 
            cx="50%" 
            cy="50%" 
            innerRadius="35%" 
                  ...
                  return (
                    <div className="bg-black/90 border border-[#28E7A2] p-2 shadow-[0_0_10px_rgba(40,231,162,0.2)]">
                      <div className="flex items-center justify-between gap-4 mb-1">
                        <span className="font-mono text-[10px] text-[#28E7A2] uppercase tracking-widest">{data.name}</span>
         ...
         <div className="flex flex-col gap-1">
            {formattedData.slice(0, 3).map((d) => (
               <div key={d.name} className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-[9px] text-[#666] uppercase tracking-wide">{d.name}</span>
      ...
      />
      <div 
        className="absolute top-0 left-0 right-0 h-[1px] bg-[#28E7A2] opacity-20 animate-[scanVertical_4s_linear_infinite]"
      />
      <style>{`
