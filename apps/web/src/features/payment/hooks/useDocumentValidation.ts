...
export function validateDocuments(payment: Payment): DocumentValidationResult {
  ...
  const tier = payment.amount < 1000 ? 'under_1000'
  ...
  const attached = (payment.manifests || []).map(m => m.type) as DocumentType[];
...
export function useDocumentValidation(payment: Payment | null): DocumentValidationResult | null {
  return useMemo(() => {
