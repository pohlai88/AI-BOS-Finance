        ...
        <div className="relative pl-4 space-y-1">
          ...
          {trace.map((step, i) => (
            ...
            >
              ...
              <div 
                className={cn(
                  "w-3 h-3 rounded-full border-2 flex-shrink-0 z-10 bg-background",
                  step.result === 'pass' && "border-primary",
              ...
              <span className={cn(
                ...
                step.result === 'pass' && "bg-primary/10 text-primary",
                step.result === 'fail' && "bg-red-500/10 text-red-400",
                step.result === 'skip' && "bg-nexus-subtle text-text-secondary",
...
const VerdictCard = ({ verdict }: { verdict: Verdict }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-surface-card border border-default p-6"
    >
      {/* Header */}
        ...
        <button
          ...
          className={cn(
            ...
            showTrace 
              ...
              : "border-default text-text-secondary hover:border-primary hover:text-primary"
          )}
        >
        ...
        {verdict.details.map((rule) => (
          <div 
            ...
            className={cn(
              ...
              rule.result.status === 'pass' && "border-primary/20 bg-primary/5",
              rule.result.status === 'warning' && "border-yellow-500/20 bg-yellow-500/5",
              rule.result.status === 'fail' && "border-red-500/20 bg-red-500/5"
            )}
          >
            {rule.result.status === 'pass' && <CheckCircle className="w-4 h-4 text-primary flex-shrink-0" />}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11px] text-text-secondary truncate">{rule.result.msg}</p>
            </div>
          </div>
        ))}
      </div>
...
export const TruthBar = () => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
        ...
        <form onSubmit={handleQuickSearch}>
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-14 bg-surface-card border border-default pl-12 pr-32 text-white font-mono placeholder:text-nexus-structure focus:border-primary focus:outline-none transition-colors"
            />
            <button
              type="submit"
              disabled={isThinking}
              className={cn(
                ...
                "border border-primary text-primary hover:bg-primary hover:text-nexus-void",
          ...
          {TRANSACTION_STREAM.slice(0, 4).map((tx) => (
            <button
              ...
              className={cn(
                ...
                selectedTx?.id === tx.id
                  ? "border-primary text-primary bg-primary/5"
                  : "border-default text-text-secondary hover:border-nexus-subtle hover:text-text-primary"
              )}
            >
              ...
              {THINKING_PHASES.map((_, i) => (
                <div
                  ...
                  className={cn(
                    ...
                    i <= thinkingPhase ? "bg-primary" : "bg-nexus-structure"
                  )}
                />
              ))}
            </div>
      ...
      {/* Initial State */}
      {!verdict && !isThinking && (
        <div className="bg-surface-card/50 border border-dashed border-default p-8 text-center">
          <div className="flex justify-center mb-4">
            <NexusIcon size="lg" animated />
          </div>
          <div className="text-text-primary mb-1">Select a Transaction to Analyze</div>
