...
export function HealthModuleCard({ module, onClick, isSelected }: HealthModuleCardProps) {
  ...
  return (
    <button
      ...
      `}
    >
      {isSelected && <div className="absolute top-0 left-0 w-1 h-full bg-white" />}

      <div className="flex items-center justify-between w-full mb-4">
        <h3 className="text-white font-medium text-lg leading-tight">{module.name}</h3>
        <div
          className={`flex items-center gap-2 px-2 py-0.5 border text-[10px] font-mono uppercase tracking-wider rounded-sm ${colorClass}`}
        >
      ...
      <div className="mt-auto pt-4 border-t border-[#1F1F1F] w-full">
        <div className="flex items-start gap-2">
          ...
          <p className="text-xs text-[#888] leading-relaxed line-clamp-2">{module.keyIssue}</p>
        </div>
      </div>
