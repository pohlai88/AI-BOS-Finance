...
export function SysProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<'identity' | 'growth' | 'connections' | 'settings'>(
    'identity',
  );
          ...
          <div className="relative">
            <NexusCard className="min-h-full h-full relative" padding="md">
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="space-y-4 pt-4">
                    ...
                    <CourseRow
                      ...
                      progress={100}
                    />
                    <CourseRow title="Advanced React Patterns" status="In Progress" progress={45} />
                  </div>

                  <div className="pt-8">
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-[12px] font-mono text-zinc-400 uppercase tracking-widest">
                        Skills & Endorsements
                      </h2>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <NexusBadge variant="neutral">
                        Leadership <span className="text-zinc-600 ml-1">12</span>
                      </NexusBadge>
                      <NexusBadge variant="neutral">
                        React Native <span className="text-zinc-600 ml-1">8</span>
                      </NexusBadge>
                      <NexusBadge variant="success">
                        Crisis Mgmt <span className="text-emerald-800 ml-1">New</span>
                      </NexusBadge>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-8 -mx-6 -mb-6 p-6 border-t border-[#1F1F1F] bg-[#0A0A0A] flex justify-end">
                <NexusButton onClick={handleSave} isLoading={isSaving} className="w-48">
                  Save Changes
                </NexusButton>
              </div>
            </NexusCard>
...
function NavButton({ active, onClick, label, desc }: any) {
  return (
    ...
    >
      <div
        className={`text-[11px] font-mono tracking-widest mb-1 ${active ? 'text-white' : 'text-zinc-500 group-hover:text-zinc-300'}`}
      >
        {label}
      </div>
...
function StatCard({ label, value, sub, highlight }: any) {
  return (
    <div className="border border-[#1F1F1F] bg-[#050505] p-4">
      ...
      <div className={`text-2xl font-mono ${highlight ? 'text-emerald-500' : 'text-white'}`}>
        {value}
      </div>
...
function CourseRow({ title, status, progress }: any) {
  return (
    <div className="flex items-center justify-between p-4 border border-[#1F1F1F] bg-[#050505]">
      ...
      <div
        className={`text-xs font-mono ${progress === 100 ? 'text-emerald-500' : 'text-zinc-500'}`}
      >
        {progress}%
      </div>
    </div>
