/**
 * POST /api/payments/[id]/execute - Execute approved payment
 * 
...
import { ExecutionInputSchema } from '@/modules/payment/schemas';
...
export async function POST(
  request: NextRequest,
  ctx: RouteContext<'/api/payments/[id]/execute'>
) {
  try {
    ...
    const validation = ExecutionInputSchema.safeParse(body);
    ...
    // Get ExecutionService and execute payment
    ...
    const result = await executionService.execute(
      ...
      validation.data.beneficiary
    );

    return NextResponse.json(result);
  } catch (error) {
    return handlePaymentError(error, 'executing');
  }
}
