...
import { tryParseURN, type ARCellCode } from './urn';
...
export class EventBroadcaster {
  ...
  async zoneTriggerFired(
    ...
    success: boolean,
    result: {
      sourceStatusUpdate?: { cell: ARCellCode; entityId: string; oldStatus?: string; newStatus: string };
  ...
  /**
   * Broadcast payment received (AR-specific)
   */
  async paymentReceived(
    ...
    const message = createWSMessage('payment:received', tenantId, {
      customerId,
      customerName,
      invoiceId,
      invoiceNumber,
    ...
    const topics = [
      ...
      TOPIC_PATTERNS.CELL(tenantId, '03'), // Receipt cell
      TOPIC_PATTERNS.CUSTOMER(tenantId, customerId),
    ];
    
    await this.broadcastToTopics(topics, message);
    await this.persistEvent('ar.payment.received', { 
      customerId, receiptId, amount, currency 
    }, { userId: 'system', tenantId });
  }

  /**
   * Broadcast aging bucket change (AR-specific)
   */
  async agingUpdated(
    customerId: string,
    customerCode: string,
    ...
    const message = createWSMessage('aging:updated', tenantId, {
      customerId,
      customerCode,
      oldBucket,
      newBucket,
    ...
    const topics = [
      TOPIC_PATTERNS.TENANT_CANVAS(tenantId),
      TOPIC_PATTERNS.CELL(tenantId, '05'), // Aging cell
      TOPIC_PATTERNS.CUSTOMER(tenantId, customerId),
      getAgingBucketTopic(tenantId, daysOverdue),
    ];
    
    // Also include old bucket topic if different
    if (oldBucket !== newBucket) {
      // Parse old bucket to get old days overdue (simplified)
      const oldDaysMap: Record<string, number> = {
        'current': 0,
        '1-30': 15,
        '31-60': 45,
        '61-90': 75,
      ...
      const oldDays = oldDaysMap[oldBucket] ?? 0;
      topics.push(getAgingBucketTopic(tenantId, oldDays));
    }
  ...
  /**
   * Broadcast collection action (AR-specific)
   */
  async collectionAction(
    ...
    promiseAmount: number | undefined,
    actor: EventActor
  ): Promise<void> {
    const message = createWSMessage('collection:action', actor.tenantId, {
      customerId,
      customerCode,
      actionType: actionType as 'phone_call' | 'email_sent' | 'letter_sent' | 'payment_promised' | 'dispute_opened' | 'escalated',
      notes,
      followUpDate,
      promiseDate,
      promiseAmount,
    ...
    const topics = [
      TOPIC_PATTERNS.TENANT_CANVAS(actor.tenantId),
      TOPIC_PATTERNS.CELL(actor.tenantId, '05'), // Aging cell
      TOPIC_PATTERNS.CUSTOMER(actor.tenantId, customerId),
    ];
    
    await this.broadcastToTopics(topics, message);
    await this.persistEvent('ar.collection.action', { 
      customerId, actionType, notes 
    }, actor);
  }
