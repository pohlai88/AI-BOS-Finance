...
export class CanvasObjectService {
  ...
  /**
   * Create a hydrated sticky from an AR entity
   */
  ...
  /**
   * Get objects by customer (for AR-specific queries)
   */
  async getObjectsByCustomer(
    customerId: string,
    actor: ActorContext
  ): Promise<CanvasObject[]> {
    // This would query by a customer-specific tag or metadata
    return this.canvasRepo.listObjects({
      tenantId: actor.tenantId,
  ...
  async addTag(
    ...
    // Check permissions - risk tags require manager+
    const isRiskTag = ['#RISK', '#OVERDUE', '#COLLECTION', '#WRITEOFF'].includes(tag.toUpperCase());
    if (isRiskTag && !['manager', 'admin'].includes(actor.role ?? '')) {
      throw new LayerPermissionError('Only managers can add risk tags');
    }
